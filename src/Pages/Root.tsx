          ...
          {data.map((item, i) => (
            <Link key={i} to={`/${item.name}`}>
              <div
                ...
                <button className=' uppercase text-[11px] font-bold tracking-[1px] leading-[1.93px]  lg:pb-[35px] md:pb-[17px] lg:pt-[40px]'>{item.name}</button>
                {/* <span className='hidden w-full h-[4px] bg-[var(--Mercury)]'></span> */}

                {palnetName === item.name && (
                  // <span className='w-full h-[4px] bg-[var(--Mercury)]'></span>
                  <span className={`p${i + 1} w-full h-[4px] `}></span>
                )}
